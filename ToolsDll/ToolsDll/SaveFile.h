...
//---------------------------------------------------------------基本操作 BasicOperation-----------------------------------------------------------
//清空字符串
TOOLSDLL_API string emptyString(string& text);

//模板 输出文本(默认0 不换行
template<typename T>
TOOLSDLL_API void output(T value, const int& i = 0);

//修改字符串中指定字符(char: ASCLL)
TOOLSDLL_API string completeString(string& text, char& charNumbers);

//执行 传入的控制台命令
extern "C" TOOLSDLL_API void consoleCommand(string text);
//---------------------------------------------------------------文件操作 FileOperations-----------------------------------------------------------


class TOOLSDLL_API FileOperations
{
private:
